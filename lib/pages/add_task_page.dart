...
class AddTaskPage extends StatefulWidget {
  ...
  const AddTaskPage({Key? key, this.existingTask, required this.onSave})
      : super(key: key);
...
class _AddTaskPageState extends State<AddTaskPage>
  ...
  TaskPriority _priority = TaskPriority.medium; // akan di-override di initState
  DateTime? _dueDate;
  int? _reminderHour;
  int? _reminderMinute;
  ...
  void initState() {
    ...
    // Load prioritas default dari settings
    if (widget.existingTask == null) _loadDefaultPriority();
    ...
    if (widget.existingTask != null) {
      ...
      _subtasks = List.from(t.subtasks.map((s) =>
  ...
  Future<void> _loadDefaultPriority() async {
    final prefs = await SharedPreferences.getInstance();
    final defaultPriority = prefs.getString('defaultPriority') ?? 'Medium';
    setState(() {
      ...
      builder: (context, child) => Theme(
        data: Theme.of(context).copyWith(
            colorScheme: const ColorScheme.light(
                primary: Color(0xFF4CAF8D), onPrimary: Colors.white)),
        child: child!,
      ),
    );
  ...
  Future<void> _pickReminderTime() async {
    final picked = await showTimePicker(
      context: context,
      initialTime:
          TimeOfDay(hour: _reminderHour ?? 9, minute: _reminderMinute ?? 0),
      builder: (context, child) => Theme(
        data: Theme.of(context).copyWith(
            colorScheme: const ColorScheme.light(primary: Color(0xFF4CAF8D))),
        child: child!,
      ),
    );
    if (picked != null) {
  ...
  void _toggleSubtask(int index) => setState(
          ...
          child: Column(
            ...
            children: [
                ...
                children: [
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                        ...
                        border: Border.all(color: Colors.black12)),
                    child:
                        const Icon(Icons.add, size: 18, color: Colors.black38),
                  ),
                ],
              ),
              const SizedBox(height: 16),
                        ...
                        title: Text(s.title,
                            style: TextStyle(
                              ...
                              decoration: s.isCompleted
                                  ...
                                  : null,
                              color:
                                  s.isCompleted ? Colors.black38 : Colors.black,
                            )),
              ...
              Container(
                ...
                child: Column(
                  children: [
                    const Text('Drag & Drop files to attach or Browse',
                    ...
                    Container(
                      ...
                      decoration: BoxDecoration(
                          border: Border.all(color: const Color(0xFFB8D8C7)),
                          borderRadius: BorderRadius.circular(8)),
                      child:
                ...
                child: GestureDetector(
                  ...
                  child: Text(
                    _dueDate != null
                        ...
                        : 'Set date',
                    style: TextStyle(
                        color: _dueDate != null ? Colors.black : Colors.black38,
                        fontSize: 13),
                  ),
                ),
              ),
                  ...
                  if (_hasReminder && _reminderHour != null)
                    GestureDetector(
                      ...
                      child: Text(_reminderText,
                          style: const TextStyle(
                              fontSize: 13, color: Color(0xFF4CAF8D))),
                    ),
                ]),
              ),
              const Divider(height: 1, color: Colors.black12),
                  ...
                  child: Text(
                    ...
                    style: TextStyle(
                      color: _recurringType != RecurringType.none
                          ...
                          : Colors.black38,
                      fontSize: 13,
                      fontWeight: _recurringType != RecurringType.none
                          ? FontWeight.w600
                          : FontWeight.normal,
                    ),
                  ),
  ...
  Widget _statusDropdown() {
    return DropdownButton<TaskStatus>(
      ...
      items: TaskStatus.values.map((s) {
        final data = {
          TaskStatus.open: ['grey', 'Open'],
          TaskStatus.inProgress: ['orange', 'In Progress'],
          TaskStatus.done: ['green', 'Done']
        };
        final colors = {
          TaskStatus.open: Colors.grey,
          TaskStatus.inProgress: Colors.orange,
          TaskStatus.done: const Color(0xFF4CAF8D)
        };
        ...
        return Column(
          ...
          children: [
            ...
            ...TaskPriority.values.map((p) => ListTile(
                  leading:
                      Icon(Icons.circle, color: info[p]![0] as Color, size: 14),
                  title: Text(info[p]![1] as String),
                  trailing: _priority == p
                      ? const Icon(Icons.check, color: Color(0xFF4CAF8D))
                      : null,
                  onTap: () {
        ...
        children: [
          ...
          ...RecurringType.values.map((r) => ListTile(
                leading: Icon(Icons.repeat,
                    color: r != RecurringType.none
                        ? const Color(0xFF4CAF8D)
                        : Colors.black38),
                title: Text(labels[r.index]),
                trailing: _recurringType == r
                    ? const Icon(Icons.check, color: Color(0xFF4CAF8D))
                    : null,
                onTap: () {
                  setState(() => _recurringType = r);
                  Navigator.pop(context);
                },
              )),
          const SizedBox(height: 16),
